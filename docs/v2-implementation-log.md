...
## 2026-09-05 — #47 CSS/HTML Realization Binding (implemented, rescoped from the old #47/#48 plan)

Scope note: the authorization replaced the pre-pause #47/#48 plan with the
#47→#50 sequence. The old "#47 scope" entry above described the realization
precheck that now belongs to #49 and the measurable repair directives that
now belong to #50; this entry records what #47 actually shipped.

1. Ownership encoded + documented: `docs/architecture/v2-generation-ownership.md`
   maps every generated artifact to its owning stage; the deterministic
   `realization` block (per-region `[data-region]` selector bindings,
   `css-defined-classes-only`, `copy-fits-measured-regions`) is planned into
   the Implementation Contract and validated to mirror the Blueprint topology
   exactly (no invented/dropped/reordered regions; selector must reference the
   canonical region id).
2. CSS vocabulary contract: the CSS call receives the binding block (must
   scope a real rule to every canonical region selector, owns the shared
   component classes); every page call receives the frozen inventory of
   classes the generated CSS actually defines and must style itself with
   those classes verbatim.
3. `REGION_STYLE_MISSING` is rule-level: a tolerant CSS parser (`parseCssRules`)
   strips comments, walks `@media`/`@supports`, and requires a real
   declaration-bearing rule whose selector realizes the binding (quote style
   and element qualification normalized; comment mentions never count).
4. `ORPHANED_CLASS` is classified: behavior/state classes (referenced by
   `site.js`, `is-*`/`has-*`/`js-*`, reserved state names) and non-styling
   hooks are exempt; any styling-intent class without a CSS rule fails the
   page. Unknown classes fail closed into styling-intent.
5. Orientation conformance: Image Slot orientation derives from the Blueprint
   role's own words (was hardcoded landscape); acceptance gate decodes
   PNG/JPEG/WEBP dimensions (`src/lib/image-dimensions.ts`) and rejects
   orientation-mismatched attempts as recorded failed attempts (bounded by
   the existing per-slot attempt cap; spend stays on the ledger; wave event
   names the rejection).
6. Informed regeneration entry point (`regeneratePagesForRealization`):
   regenerates only affected pages from measured finding directives under
   NEW immutable subkeys (`{page}.realization-repair-1`), reuses the frozen
   shared source + unaffected pages verbatim, is idempotent under workflow
   retry (reuse-path bug found by test and fixed), re-validates the whole
   assembled site, and never consumes the QA repair budget. Wiring into the
   pipeline lands with #49's preflight placement.
7. Frozen negative regression: the v3 fixture (30 styling-intent orphaned
   classes incl. the full-bleed hero and 4-up grid vocabulary; 9/9 canonical
   regions without any scoped CSS rule) fails deterministically at assembly.
   Fixture content is transported via `tests/_generated-realization-fixture.ts`
   (generated by vitest.config.ts — the worker isolate cannot read the repo
   filesystem).

Evidence: 41 test files / 306 tests green (baseline 39/280); `tsc --noEmit`
