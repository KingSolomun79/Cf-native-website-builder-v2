...
- **H2 wiring (#28, staging-equivalent complete; production deploy-coupled):** the transport endpoint now exists as the `wazibiz-email-router` Worker (`src/email-router/`, deployed live with the shared `WAZIBIZ_EMAIL_TRANSPORT_TOKEN` secret; fail-closed 503 until the operator sets `SMTP2GO_API_KEY`). `WAZIBIZ_EMAIL_TRANSPORT_URL` points at it in `wrangler.jsonc`, and same-account delivery flows through the `EMAIL_ROUTER` service binding (Workers cannot fetch each other's `*.workers.dev` URLs within one account — live-verified as infrastructure 404s). Bounded transient retry now has a production trigger (`*/10` cron sweep). Controlled staging evidence (`cf-website-factory-staging` + `website_factory_staging` D1): acceptance → Email Delivery through the live router with visitor email as Reply-To and the platform Sender Identity, plus both fail-closed states (URL unset → transient "not configured"; router provider leg unset → transient "transport responded 503"). Remaining production steps, gated on the #27 runbook deploy: set `WAZIBIZ_EMAIL_TRANSPORT_TOKEN` on `cf-website-factory`, set `SMTP2GO_API_KEY` on `wazibiz-email-router`, configure a real verified Sender Identity. CSO: `docs/security/2026-09-01-v2-issue-28-cso.md` (watch items W-28a-f).
