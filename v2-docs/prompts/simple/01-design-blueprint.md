...
- **Tokens**: hex color roles (ground, ink, secondary text, accent, hairlines, error), type families with free fallbacks and weight guidance, a real type scale using clamp() sizes with line-heights and max ch widths, radius rule, shadow policy, texture, container width, section spacing, image treatment.
...
- **Imagery**: one consistent grade (lighting/color world), then image slots with stable kebab-case ids. Each slot: page, section, priority (CRITICAL = the design collapses without it), aspect ratio, subject direction, composition direction, lighting, palette, crop behavior, alt text — and a ready-to-use `kiePrompt` (concrete photographic description, 1–3 sentences, no readable text/logos/lettering in the image) plus `negativePrompt`. Only slots the design actually needs (roughly 4–12 for a typical four-page site).
