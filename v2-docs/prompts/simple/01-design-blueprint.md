...
- **Four page specs**: every major HOME section in order — purpose, layout, approximate visual mass, surface, typography, media, CTA, responsive behavior. Then ABOUT, SERVICES and CONTACT derived from the same DNA (the reference may not contain exact analogues; derive inner-page treatments, do not invent reference pages). FOUR-PAGE HERO MEDIA — REQUIRED FIELD (output without it is REJECTED): for EVERY page — home, about, services AND contact — the FIRST object in `sections` is the hero and MUST carry a non-empty `mediaSlotId` string naming that page's DEDICATED HERO IMAGE SLOT — the slot whose `page` is that page and whose `section` is that page's hero section (section starts with "Hero", e.g. `"section": "Hero"`), normally ids like `home-hero`, `about-hero`, `services-hero`, `contact-hero`. The hero section and the hero slot must match: never point a page hero at a body/content slot (an about-split, story, gallery or atmosphere slot), and never at another page's slot. A blueprint whose hero sections omit `mediaSlotId` or point at non-hero slots is deterministically rejected. The invariant: no routed page opens with a large empty/typography-only header — every page gets meaningful photographic visual mass above the fold, in whatever treatment the Reference design language indicates (full-width photographic background with text overlay, split hero with large photograph, photographic band with inset content panel, oversized image beside headline, full-bleed image with dark/color wash). Home keeps its ~one-viewport hero mass; inner-page heroes carry roughly 0.45–0.75 viewport of hero mass on desktop (do not hardcode these proportions when the Reference strongly indicates another one) and keep meaningful media height on mobile (about 35–50svh) — never tiny banner crops.
