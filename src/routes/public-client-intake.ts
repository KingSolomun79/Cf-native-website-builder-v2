...
export async function submitClientIntake(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) {
    return c.json({ error: "Body size out of allowed range" }, 413);
  }
  ...
  if (!validated.valid) {
    return c.json({ error: { code: "DRAFT_INVALID", issues: validated.issues } }, 400);
  }
  ...
  if (!turnstileToken || !(await verifyTurnstile(c.env, turnstileToken))) {
    return c.json({ error: { code: "TURNSTILE_FAILED", message: "Turnstile verification failed" } }, 403);
  }
  ...
  if ((recent?.hit_count ?? 0) >= RATE_LIMIT_MAX_PER_WINDOW) {
    return c.json({ error: { code: "RATE_LIMITED", message: "Too many submissions; try again later" } }, 429);
  }
  if (recent) {
