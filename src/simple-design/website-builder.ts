...
${vocabularyBlock}${chrome ? `${chromePromptBlock(chrome)} The header and footer are the FROZEN SHARED CHROME — reproduce them exactly (byte-for-byte; live rejection: a services header that drifted from the frozen chrome failed the build); only <main> (and the page-specific <head> metadata) is this page's own.\n\n` : ""}THIS PAGE'S MANDATORY CRITICAL IMAGES (part of the ledger above — every listed slot MUST appear on this page, in its section, by exact slot id):
